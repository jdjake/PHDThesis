...
Let us now more precisely describe the problem of this thesis. Let $X$ be a compact manifold equipped with a volume density, and let $P$ be an elliptic operator on $X$ of order $s > 0$, formally positive-definite in the sense that
%
...
To study the regularity of $a(P)$, we introduce the spaces $M^{p,q}(X)$, consisting of all functions $a: \Lambda \to \CC$ for which the operator $a(P)$ is  bounded from $L^p(X)$ to $L^q(X)$. The space $M^{p,q}(X)$ is then equipped with a norm
%
\[ \| a \|_{M^{p,q}(X)} = \sup \left\{ \frac{\| a(P) f \|_{L^q(X)}}{\| f \|_{L^p(X)}} : f \in C^\infty(X) \right\}. \]
%
For notational convenience, we write $M^p(X) = M^{p,p}(X)$. Orthogonality implies $M^2(X)$ is isometrically equal to $l^\infty(\Lambda)$, but the structure of the other spaces $M^{p,q}(X)$ is not immediately clear.
...
The discrete spectrum of the operator $P$ makes it difficult to apply methods of oscillatory integrals to the problem. Fortunately, certain semiclassical heuristics tell us that these problems disappear when we restrict the problem to 'high frequency inputs'. In order to take advantage of this fact, rather than studying what conditions ensure the operators $a(P)$ are bounded, we study conditions that ensure the operators $a_R(P) = a(P/R)$ are uniformly bounded as $R \to \infty$. %, since these operators depend more and more on 'high frequency behaviour' in the limit.
To prevent pathological examples from arising under dilation, we restrict ourselves to \emph{regulated functions} $a$, i.e. functions $a: [0,\infty) \to \CC$ such that
%
...
Then define $M^{p,q}_{\text{Dil}}(X)$ to be the norm space of all regulated functions $a$ such that the norm $\| a \|_{M^{p,q}_{\text{Dil}}(X)} = \sup_R \| a_R \|_{M^{p,q}(X)}$ is finite.

With notation introduced, we now state precisely the problem studied in this thesis:
...
\end{changemargin}
%
Before the results of this thesis, no such results were known for $X \neq \TT^d$, aside from the isometry $M^2_{\text{Dil}}(X) = L^\infty[0,\infty)$, which holds by orthogonality. We will obtain results under two main assumptions, which we call Assumption A and Assumption B, the first being a curvature condition on the principal symbol of the $P$, and the second an assumption about the operator's eigenvalues.
...
When $P = \sqrt{-\Delta}$ on a Riemannian manifold $M$, Assumption A always holds, because the cospheres defined above are ellipsoids. Even in the more general setting, Assumption A implies a geometry on the manifold $M$, and Assumption B is related to this geometry, in particular, that geodesics with respect to this geometry are all closed and have common length. 

Under these assumptions, we find necessary and sufficient conditions for a function to be contained in $M^{p,q}_{\text{Dil}}(M)$ for $1/p - 1/2 > 1/(d+1)$ and for $p \leq q < 2$. More precisely, fix $\chi \in C_c^\infty(\RR^d)$ with $1 = \sum \chi(t / 2^j)$ for all $t \neq 0$. For $s,\alpha \geq 0$ and $1 \leq q \leq \infty$, define the norm
%
\[ \| a \|_{R^{s,q}_\alpha[0,\infty)} = \sup\nolimits_{j \in \ZZ} 2^{-jd \alpha} \left( \int_0^\infty \left| \widehat{a}_j(t) \langle t \rangle^s \right|^q\; dt \right)^{1/q}, \]
%
where $a_j(t) = \chi(t) a(2^j t)$, and $\widehat{a}_j(t) = \int a(\lambda) \cos(2 \pi \lambda t)$ is the cosine transform of $a$, i.e. the Fourier transform of the even extension of $a$ to a function on $\RR$. Then if $s = (d-1)(1/q - 1/2)$, and $\alpha = 1/p - 1/q$, we will prove that $\| a \|_{M^{p,q}_{\text{Dil}}(X)} \sim \| a \|_{R^{s,q}_\alpha[0,\infty)}$ for any regulated function $a$. One may view control on the $R^{s,q}_\alpha$ norm as a smoothness condition on the multiplier $a$; indeed the Hausdorff-Young inequality implies a Sobolev space estimate
%
\[ \| a_j \|_{W^{s,q'}[0,\infty)} \lesssim 2^{jd \alpha} \| a \|_{R^{s,q}_\alpha[0,\infty)}. \]
%
However, one cannot characterize the space $R^{s,q}_\alpha[0,\infty)$ in terms of usual spaces which measure smoothness. Nonetheless, we do have the Besov space estimate
%
\[ \| a \|_{R^{s,q}_\alpha[0,\infty)} \lesssim \sup\nolimits_j 2^{-jd \alpha} \| a_j \|_{B^{s+1/q - 1/2,2}_q[0,\infty)}. \]
%
Inequality TODO tells us that elements of $R^{s,q}_\alpha$ have $s$ derivatives in $L^{q'}$, though with `something extra' occuring on the other side of the Fourier transform. But Sobolev embedding heuristics tells us that having $s + 1/q - 1/2$ derivatives in $L^2$ implies having $s$ derivatives in $L^{q'}$, and the presence of $L^2$ norms on the right hand side of this inequality, allows us to losslessly convert between estimates on the Fourier transform side, which allows us to recover that `something extra' needed to be contained in $R^{s,q}_\alpha[0,\infty)$.
...
%
Thus a multiplier of $\Delta$ on $\TT^d$ is nothing more than a Fourier multiplier operator on $\TT^d$ whose symbol is radial. Methods of transference\footnote{See Section 3.6.2 of Grafakos \cite{Grafakos}, based on methods of de Leeuw \cite{deLeeuw}} show that $\| a \|_{M^{p,q}(\RR^d)} \sim \| a \|_{M^{p,q}_{\text{Dil}}(\TT^d)}$, where $\| a \|_{M^{p,q}(\RR^d)}$ is the operator norm of the Fourier multiplier on $\RR^d$ given by
%
\[ Tf(x) = \int_{\RR^d} a \big(|\xi| \big) \widehat{f}(\xi) e^{2 \pi i \xi \cdot x}\; d\xi \]
%
from $L^p(\RR^d)$ to $L^q(\RR^d)$. Thus in this section we will focus on operators of this type, which we call \emph{radial Fourier multiplier operators}.
...
%
\[ \mathcal{B}_d a(r) = (2 \pi) r^{1-d/2} \int_0^\infty a(\lambda) J_{d/2 - 1}(2 \pi \lambda r) \lambda^{d/2}\; d\lambda. \]
%
...
\begin{theorem} \label{GarrigosSeegerTheorem}
    ...
    %
    \[ \left( \int_0^\infty |\mathcal{B}_d a (t)|^p t^{d-1} \right)^{1/p} \sim_{p,d,\phi} \left( \int_0^\infty |\mathcal{C} a(t)|^p \langle t \rangle^{(d-1)(1 - p/2)}\; dt \right)^{1/p}, \]
    %
    where $\mathcal{C} a(t) = \int_0^\infty a(\lambda) \cos(2 \pi \lambda t)\; dt$ is the cosine transform of $a$.
\end{theorem}
...
The general study of the boundedness properties of Fourier multiplier operators in multiple variables was initiated in the 1950s, as connections of the theory to partial differential equations became more fully realized\footnote{See \cite{Hormander1} for a discussion of what was discovered at this time.}. It was quickly realized that the most fundamental estimates were $L^p \to L^q$ estimates for such operators.
%
%for $1 \leq p \leq 2$, and $q \geq p$, which by duality are equivalent to bounds
%
%\[ \| Tf \|_{L^{p^*}(\RR^d)} \lesssim \| f \|_{L^{q^*}(\RR^d)}, \]
%
It is therefore natural to introduce the space $M^{p,q}(\RR^d)$, consisting of all symbols $m$ which induce a Fourier multiplier operator $T$ bounded from $L^p(\RR^d)$ to $L^q(\RR^d)$, with the operator norm of $T$ giving a norm on the space. The transference principles mentioned in the previous section imply that for any regulated function $a$,
%
...
%
\begin{itemize}
    \item The spaces $M^{1,q}(\RR^d) = M^{q^* \!\! ,\infty}(\RR^d)$ can be characterized, by virtue of the fact that the boundedness of operators with domain $L^1(\RR^d)$ or range $L^\infty(\RR^d)$ is often simple; we have
    %
    \[ M^{1,q}(\RR^d) = \widehat{L^q}(\RR^d) \quad\text{for $q > 1$,}\quad \text{and}\quad M^{1,1}(\RR^d) = \widehat{M}(\RR^d), \]
    %
    Here $M(\RR^d)$ is the space of all finite signed Borel measures, equipped with the total variation norm. The proof follows from Schur's Lemma for integral operators, which often gives tight estimates when bounding for operators in terms of the $L^1$ norm of their input.
...
Despite the continuing lack of a complete characterization of the classes $M^{p,q}(\RR^d)$, it is surprising that we \emph{can} conjecture a characterization of the subspace of $M^{p,q}(\RR^d)$ consisting of \emph{radial symbols}, for an appropriate range of exponents. The conjectured range of estimates was first suggested by the result of \cite{GarrigosandSeeger}, which concerned the boundedness of a radial Fourier multiplier $T = a(P)$ \emph{restricted to radial functions}, i.e. such that the norm
%
\[ \| a \|_{M^{p,q}_{\text{Rad}}(\RR^d)} = \sup \left\{ \frac{\| a(P)f \|_{L^q(\RR^d)}}{\| f \|_{L^p(\RR^d)}} : \text{$f$ is radial} \right\} \]
%
is finite. For any $\chi \in C_c^\infty(\RR)$, if we define $k_t$ to be the Fourier transform of $\chi(|\cdot|) a(t |\cdot|)$, then the identity $k_t = a(tP) \widehat{\chi}$ and dilation symmetries imply that if $\alpha = d(1/p - 1/q)$, then
%
\[ \| k_t \|_{L^q(\RR^d)} \lesssim t^\alpha \| a \|_{M^{p,q}_{\text{Rad}}(\RR^d)}, \]
%
with implicit constants depending on $\chi$. For $d > 1$, for $1/p - 1/2 > 1/2d$, and for $q \in [p,2]$, Garrigos and Seeger proved the converse bound
%
\begin{equation} \label{RadialMultiplierRadialBound}
    \| a \|_{M^{p,q}_{\text{Rad}}(\RR^d)} \lesssim \sup\nolimits_{t > 0} t^{-\alpha} \| k_t \|_{L^q(\RR^d)},
\end{equation}
%
Theorem \ref{GarrigosSeegerTheorem} implies that
%
\[ \sup\nolimits_{t > 0} t^{- \alpha} \| k_t \|_{L^q(\RR^d)} \sim \| a \|_{R^{s,q}_\alpha[0,\infty)}, \]
%
where $s = (d-1)(1/q - 1/2)$. Thus in the range above, $M^{p,q}_{\text{Rad}}(\RR^d) = R^{s,q}_\alpha[0,\infty)$.
...
For any other value of $p$ and $q$, $M^{p,q}_{\text{Rad}}(\RR^d)$ is a proper subset of $R^{s,q}_\alpha[0,\infty)$, as the following counterexamples show:
%
\begin{itemize}
  \item For $p = 1$, the inclusion `fails by a logarithm', as we can see by the characterization of $M^{1,q}(\RR^d)$ in the last section; if $a$ is supported on an interval $I$ we have the converse inequality $\| a \|_{M^{1,q}(\RR^d)} \lesssim \log |I|^{1/q} \| a \|_{R^{s,q}_\alpha[0,\infty)}$ whose dependence on $I$ is in general sharp, and cannot be removed.

  \item Suppose $p \geq 2d/(d+1)$. Then $R^{s,q}_\alpha[0,\infty)$ contains $B^{0.5,2}_p[0,\infty)$, and thus, in particular, must contain unbounded functions for $p > 1$ (essentially because the Sobolev embedding theorem fails when used at the endpoint to embed into $L^\infty$), and by dilation symmetry, must contain unbounded functions supported on $[1/2,2]$. Since $M^p(\RR^d) \subset M^2(\RR^d)$, $M^p(\RR^d)$ cannot contain unbounded functions, and thus $M^p(\RR^d)$ is a proper subset of $R^{s,p}_\alpha(\RR^d)$. TODO $M^{p,q}(\RR^d)$ boundedness.
...
It is natural to conjecture that the same constraint continues to hold when we remove the constraint that our inputs $f$ are radial, i.e so that for a radial function $m(\xi) = a(|\xi|)$, for $d > 1$, $1/2d < 1/p - 1/2 < 1/2$, and for $p \leq q < 2$, if $s = 1/p - 1/q$ then
%
\begin{equation} \label{RadialMultiplierBound}
  \| m \|_{M^{p,q}(\RR^d)} \sim_{p,q,d} \| a \|_{R^{q,s}_d[0,\infty)}.
\end{equation}
%
...
\begin{theorem} \label{HeoNazarovSeegerTheorem}
    Suppose $1 < p < 2(d-1)/(d+1)$, and $m(\xi) = a(|\xi|)$ is a radial function. Then for $p \leq q \leq 2$, $\| m \|_{M^{p,q}(\RR^d)} \lesssim \| a \|_{R^{q,s}_d[0,\infty)}$, where $s = 1/p - 1/q$.
\end{theorem}

We begin by establishing a single scale version of this result, i.e. that if the Fourier transform of a convolution  kernel $k$ is supported on the annulus $|\xi| \sim 1$, then
%
...
\begin{proof}[Proof of \eqref{HeoNazarovSeegerSingleScaleInequality} from Lemma \ref{lemma1}] Suppose $k(\cdot) = b(|\cdot|)$ for some function $b: [0,\infty) \to \CC$. If we set $a(x,r) = f(x) b(r)$ for any function $f: \RR^d \to \CC$, then
%
...
%
\[ \| k * \psi \|_{L^p(\RR^d)} = \| T(k * \psi * f) \|_{L^p(\RR^d)} \lesssim \| k * \psi * f \|_{L^p(\RR^d)} \lesssim \| k \|_{L^p(\RR^d)} \| f \|_{L^p(\RR^d)}. \qedhere \]
\end{proof}
...
%
\[ \| F^m \|_{L^p(\RR^d)} \lessapprox 2^{m/(d+1)} 2^{-m(1/p - 1/2)} \left( \sum\nolimits_k 2^{k(d-1)} \#(\mathcal{E}_k) \right)^{1/p}, \]
%
where the implicit constant can depend polynomially on $m$. Thus, in the range $1/p - 1/2 > 1/(d+1)$, we may sum in $m$ using the triangle inequality to obtain a bound on $F = \sum\nolimits_m F^m$, i.e. so that
%
\[ \| F \|_{L^p(\RR^d)} \lesssim \left( \sum\nolimits_k 2^{k(d-1)} \#(\mathcal{E}_k) \right)^{1/p}, \]
%
...
\begin{equation} \label{individualscaleoperatorbound}
    \| T_j \|_{L^p(\RR^d) \to L^q(\RR^d)} \lesssim 2^{jd(1/p - 1/q)} \| k_j \|_{L^q(\RR^d)}.
\end{equation}
%
...
%
\begin{align*}
  \left\| \sum\nolimits_{j,W} T_j a_{j,H,W} \right\|_{L^1(\RR^d)} &\lesssim |\Omega_H|^{1/2} \left\| \sum\nolimits_{j,W} T_j a_{j,H,W} \right\|_{L^2(\RR^d)} \lesssim H |\Omega_H|^{1 + 1/p - 1/q}.
\end{align*}
%
Real interpolation allows us to sum in $H$, and conclude that
%
\[ \| Tf \|_{L^q(\RR^d)} = \left\| \sum\nolimits_{H,j,W} T_j a_{j,H,W} \right\|_{L^q(\RR^d)} \lesssim \left( \sum \left[ H |\Omega_H|^{1/p} \right]^p \right)^{1/p} \lesssim \| S f \|_{L^p(\RR^d)} \lesssim \| f \|_{L^p(\RR^d)}. \]
%
which completes the proof of boundedness.

Such a simple proof will not suffice for our analysis, since the class of multipliers we are considering is not pseudolocal at all --- the kernels $k_j$ are only assumed to be bounded in $L^q(\RR^d)$, and need not satisfy any decay bound as $|x| \to \infty$ (though the methods can be used to bound appropriately local operators, i.e. obtaining a different proof of the endpoint results of \cite{SeegerSingular}).

We will, however, be able to exploit the above calculation to control the close range interactions, and rely on a different argument to control large range interactions. The most elegant formulation of the long range interaction bounds are provided in the paper \cite{HeoandNazarovandSeeger2}. The idea is to take a singular scale estimate, and upgrade this to an estimate with a geometrically decaying constant term when our inputs have amplitudes that are locally constant on a much larger scale than is required by the uncertainty principle and the frequency support of the inputs, namely, that at a unit frequency scale,
%
\[ \left\| \sum\nolimits_H \sum\nolimits_{W \in \mathcal{W}_{H,l}} T_{\text{Long}} * a_{H,W} \right\|_{L^q(\RR^d)} \lesssim 2^{-l \varepsilon} \left( \sum\nolimits_H \sum\nolimits_{W \in \mathcal{W}_{H,l}} \| a_{H,W} \|_{L^\infty(\RR^d)}^p |W| \right)^{1/p}. \]
%
Summing in frequencies using $L^p$ orthogonality, we find that
%
\[ \left\| \sum\nolimits_j \sum\nolimits_H \sum\nolimits_{W \in \mathcal{W}_{H,l}} T_{j,\text{Long}} a_{j,H,W} \right\|_{L^q(\RR^d)} \lesssim 2^{-l \varepsilon} \left( \sum\nolimits_j \sum\nolimits_H \sum\nolimits_{W \in \mathcal{W}_{H,l}} \| a_{j,H,W} \|_{L^\infty(\RR^d)}^p |W| \right)^{1/p}. \]
%
Since $\| a_{j,H,W} \|_{L^\infty(\RR^d)} \lesssim H$, and since the sum is over disjoint $W$, we bound this quantity by $2^{-l \varepsilon} \left( \sum\nolimits_H H^p |\Omega_H| \right)^{1/p} \lesssim 2^{-l \varepsilon} \| f \|_{L^p(\RR^d)}$. But now we can sum in $l$ using the triangle inequality to bound the long range interactions in full. TODO FIX THIS.
...
\section{Boundedness of Quasi-Radial Multipliers}

Kim \cite{KimQuasiradial} has extended 
the bounds of Heo-Nazarov-Seeger to quasiradial multipliers, i.e. multipliers of the form $(a \circ r)(\xi)$, where $r: \RR^d \to [0,\infty)$ is a smooth, homogeneous function of order one, such that the cosphere $S = \{ \xi : r(\xi) = 1 \}$ is a hypersurface with non-vanishing Gauss curvature. Such multipliers retain the translation and dilation symmetries of radial multipliers, but are no longer \emph{rotation-invariant}.

Using the same identities as for radial Fourier multiplier operators, one can verify that
%
\[ \| a \circ r \|_{M^{p,q}(\RR^d)} \gtrsim \sup\nolimits_j 2^{-jds} \| k_j \|_{L^q(\RR^d)}, \]
%
where $s = 1/p - 1/q$, and $k_j$ is the Fourier transform of $\chi(\cdot) (a \circ r)(2^j \cdot)$, with $\chi$ as in that section. In an analysis of Bochner-Riesz type quasi-radial multipliers, Lee and Seeger \cite{LeeSeeger2} showed that
%
\[ \sup\nolimits_j 2^{-jd(1/p - 1/q)} \| k_j \|_{L^q(\RR^d)} \sim \| a \|_{R^{q,s}_d[0,\infty)}, \]
%
Kim \cite{KimQuasiradial} has verified that in the range $1/p - 1/2 > 1/(d+1)$ and $p \leq q < 2$,
%
...
The proof is an adaption of the proof of \cite{HeoandNazarovandSeeger2}. However, it is more difficult to work directly with the kernels $k_j$ in this problem, since, unlike for radial multipliers, whose kernels are also radial, the kernels $k_j$ need not be quasi-radial. It is here that we introduce a technique that has proved essential to deeper analysis of multiplier operators in settings lacking the full symmetry of Euclidean space: a reduction of analysis to the wave equation. Using the Fourier inversion formula, we write
%
\begin{align*}
...
\end{align*}
%
where $\widehat{w}_t(\xi) = e^{2 \pi i t r(\xi)}$. As $t$ varies, $u = w_t * f$ solves the differential equation $\partial_t u = 2 \pi i P u$, where $P$ is the Fourier multiplier operator whose symbol is the function $r$. Define $\chi_{x,t} = \text{Trans}_x \left( \int_{-\infty}^\infty \psi(s) w_{t - s}\; ds \right)$, where the Fourier transform of $\psi$ is non-negative and vanishing to high order at the origin. Then, using oscillatory integral techniques akin to Lemma \ref{lemma4}, one can obtain inner product estimates on the quantities $\langle \chi_{x_1,t_1}, \chi_{x_2,t_2} \rangle$ that show such terms are negligible unless $|x_1 - x_2| + |t_1 - t_2| \lesssim 1$. These estimates are sufficient to follow the proof of \cite{HeoandNazarovandSeeger2}, performing a density decomposition using the Euclidean metric on $\RR^{d+1}$, and thus obtain single scale bounds for the quasi-radial multipliers. An atomic decomposition analogous to that discussed in Section \ref{sec:combining_scales_with_atomic_decompositions} then yields Kim's result.
...
TODO: Connection between local smoothing and multiplier estimates (maybe Mockenhaupt Seeger Sogge)
...
As alluded to in the last section, we now plan to study spectral multipliers of an elliptic operator $P$ on a compact manifold $M$ via the use of solutions of the wave equation $\partial_t u = 2 \pi i P u$ on $M$, which allow us to exploit geometric information about the manifold $M$ and thus extend the results of Heo, Nazarov, and Seeger to the setting of compact manifolds when Assumption A and Assumption B of Chapter \ref{cha:multipliers_of_an_elliptic_operator} hold. When $P = \sqrt{-\Delta}$ is induced by a Riemannian metric on $M$, then the geometry that we will consider is precisely the Riemannian geometry of $M$, but given a more general operator $P$, we obtain a more general \emph{Finsler geometry} on $M$. We describe the bare essentials of Finsler geometry needed to understand the arguments that occur later on in the thesis.

Let $M$ be a $d$-dimensional manifold. We denote an element of the tangent bundle $TM$ by $(x,v)$, where $x \in M$ and $v \in T_x M$. A \emph{Finsler metric} on $M$ is a homogeneous function $F: T M \to [0,\infty)$, which is smooth on $TM - 0$, and strictly convex, in the sense that for $(x,v) \in TM - 0$, the Hessian matrix with entries
%
\begin{equation} \label{FinslerMetricCoefficients}
    g_{ij}(x,v) = \frac{1}{2} \frac{\partial^2 F^2}{\partial v^i \partial v^j}(x,v)
\end{equation}
%
is positive-definite. Then for each $x$ and $v$, gives rise to an inner product $\langle \cdot, \cdot \rangle_{(x,v)}$ on $T_x M$ which approximates the Finsler metric near $v$ up to second order.
...
Given a metric $F$ on $M$, we define a dual metric $F_*: T^*M \to [0,\infty)$ by setting $F_*(x,\xi) = \sup \{ \xi(v) : F(x,v) = 1 \}$. Then $F_*$ is also strictly convex, in the sense that for an element $(x,\xi) \in T^* M - 0$, the Hessian matrix with entries
%
\begin{equation}
    g^{ij}(x,\xi) = \frac{1}{2} \frac{\partial^2 F_{\! \! *}^2}{\partial \xi_i \partial \xi_j}(x,\xi)
\end{equation}
%
is positive definite, and so the Finsler metric gives rise to an inner product on $T_x^* M$ for each $(x,\xi) \in T^* M$ which approximates $F_*$ up to second order.

If $M$ is a Finsler manifold, we define the \emph{Legendre transform} $\mathcal{L}: T M - 0 \to T^* M - 0$, by defining it's behaviour as a homogeneous diffeomorphism $\mathcal{L}_x$ from $T_x M - \{ 0 \} \to T^*_x M - \{ 0 \}$ on each fiber. We define the map so that for $v,w \in T_x M$, $\langle \mathcal{L}_x(v), w \rangle = \langle v, w \rangle_{(x,v)}$. In coordinates, $\mathcal{L}_x(v)_i = \sum\nolimits_j g_{ij}(x,v) v^j$. If $\xi = \mathcal{L}_x(v)$, then the matrix with entries $g_{ij}(x,v)$ is the inverse of the matrix with entries $g^{ij}(x,\xi)$, so that the inverse of $\mathcal{L}_x$ is given in coordinates by $(\mathcal{L}_x^{-1} \xi)^i = \sum\nolimits_j g^{ij}(x,\xi) \xi_j$. The Legendre transform is the Finsler variant of the musical isomorphism in Riemannian geometry, though the Legendre transform is in general only \emph{homogeneous}, rather than linear. % (TODO Special type of Finsler manifolds where the transform is linear).

Now let $P$ be an elliptic pseudodifferential operator of order one on a manifold $M$, which satisfies Assumption A. The principal symbol of $P$ is a well-defined coordinate independent function $p: T^* M \to [0,\infty)$, and the following lemma applies.
...
A Finsler metric gives a length to each tangent vector on the manifold, and can thus be used to define the lengths of curves $c: I \to U_0$ by the formula $L(c) = \int_I F(c,\dot{c})\; dt$. An analysis of minimal length curves leads naturally to a study of geodesics on a Finsler manifold. The theory of geodesics on Finsler manifolds is similar to the Riemannian case, except for the interesting quirk that a geodesic from a point $p$ to a point $q$ need not necesarily be a geodesic when considered as a curve from $q$ to $p$, and so we must consider \emph{forward} and \emph{backward} geodesics. We define the \emph{forward distance} $d_+: M \times M \to [0,\infty)$ by taking the infima of paths between points. This function is a \emph{quasi-metric}, as it satisfies the triangle inequality, but is not necessarily symmetric. We define the \emph{backward distance} $d_-: M \times M \to [0,\infty)$ by setting $d_-(p,q) = d_+(q,p)$. A \emph{constant speed speed forward geodesic} on a Finsler manifold is a curve $c: I \to U_0$ which satisfies the geodesic equation
...
\end{equation}
%
equations that reoccur in the study of wave propogation, and thus tell us that high frequency wave packet solutions to the equation $\partial_t u = 2 \pi i P$ approximately travel on geodesics of the Finsler metric. Sufficiently short geodesics on a Finsler manifold are length minimizing. In particular, there exists $r > 0$ such that if $c$ is a geodesic between two points $x_0$ and $x_1$, and $L(c) < r$, then $d_M^+(x_0,x_1) = L(c)$.
...
We now return to the study of spectral multipliers on compact manifolds. In a certain sense, one can think of the Euclidean situation studied in the last Chapter as a limiting case of multipliers in the `high frequency limit'. In a heuristic sense, if we study the operator $P/R$ as $R \to \infty$ on the manifold, then as $R \to \infty$ the Finsler manifolds $M_R$ given by the Finsler metrics induced by $P/R$ converge locally around $x_0$, in a rough sense, to $\RR^d$, with metric induced by the norm $p(x_0,\cdot)$. In particular, a transference principle of Mitjagin \cite{Mitjagin} follows this heuristic (See \cite{KenigStantonTomas} for a similar result written in English) shows that if $X$ is a compact manifold equipped with an elliptic operator $P$ with principal symbol $p$, then for any regulated function $a$ and each $x_0 \in X$,
%
\[ \| a(p(x_0,\cdot)) \|_{M^{p,q}(\RR^d)} \lesssim_{X,p,q} \| a \|_{M^{p,q}_{\text{Dil}}(X)}. \]
%
As we saw in Section 2.6 (TODO), if $P$ satisfies Assumption A, it thus follows that
%
\[ M^{p,q}_{\text{Dil}}(X) \subset M^{p,q}(\RR^d) \subset R^{q,s}_\alpha(\RR^d), \]
%
with $s = (d-1)(1/q - 1/2)$ and $\alpha = d(1/p - 1/q)$.

As for radial multipliers, we might conjecture that if Assumption A holds, then for the same range of $p$ and $q$ under which the radial multiplier conjecture is believed to hold, we also have $R^{q,s}_\alpha(\RR^d) = M^{p,q}_{\text{Dil}}(X)$. Note that for any compact manifold $X$ this conjecture is strictly harder then the radial multiplier on $\RR^d$, since the inclusion TODO, would show that if $M^{p,q}_{\text{Dil}}(X) = R^{q,s}_\alpha(\RR^d)$, then $M^{p,q}(\RR^d) = R^{q,s}_\alpha(\RR^d)$.
...
For any operator $P$ to which we can establish a suitably robust functional calculus, the identity
%
\[ a(P) = \int_{-\infty}^\infty \widehat{a}(t) e^{2 \pi itP} \]
%
holds for regulated functions $a$, simply by plugging $P$ into the usual Fourier inversion formula. As $t$ varies, the operators $e^{2 \pi itP}$ solve the wave equation $\partial_t u = 2 \pi it P$, which gives hope of geometrically localizing the operator $a(P)$ by localizing the $t$ variable in TODO.

In general, it seems quite difficult to understand the behaviour of the operator $e^{2 \pi i t P}$ given it's abstract definition via the functional calculus. However if $P$ is elliptic, then $e^{2 \pi i t P}$ is a \emph{Fourier integral operator}, which roughly means that the kernel of the operator is well approximated by oscillatory integral representation \emph{for high frequency inputs}. In this section, we briefly describe the relevant components we will need from the theory of Fourier integral operators.
...
\section{Relation Between Assumption B and Periodic Geodesics}
