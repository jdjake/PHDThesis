...
\noindent \fbox{\parbox{\textwidth}{\textbf{Assumption} $\textbf{Wave-Bound}(P,p)$: There exists a constant $C_0 > 0$ such that
%
\begin{align*}
...
In Chapter \ref{chap:spectralatomicdchapter}, we verify that $\text{Wave-Bound}(p)$ holds for $1/p - 1/2 > 1/(d-1)$ when $P = P_{\text{SH}}$. Theorem \ref{maintheoremsphere} then follows from Theorem \ref{atomicscalestheorem}.% We thus obtain the following characterization of boundedness.
