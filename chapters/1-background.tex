...
\noindent \fbox{\parbox{\textwidth}{\textbf{Assumption C}: The operators $\cos(2 \pi t P)$ on $X$ defined by the functional calculus above have `finite propagation speed', in the sense that for one (and thus all) Riemannian metrics on $X$, there exists $C > 0$ such that for all $t \in \RR$, the kernel of the spectral multiplier operators $\cos(2 \pi t P)$ is supported on $\{ (x,y): d(x,y) \leq C t \}\}$.}}
...
\end{align*}
%
uniformly in $k$, $\mathfrak{b}$, and $\mathfrak{u}$, where $s = (d-1)(1/q - 1/2)$.
%
%\begin{align*}
...
In Chapter \ref{chap:spectralatomicdchapter}, we verify that $\text{Wave-Bound}_p(P)$ holds for $1/p - 1/2 > 1/(d-1)$ when $P = P_{\text{SH}}$. Theorem \ref{maintheoremsphere} then follows from Theorem \ref{atomicscalestheorem}.% We thus obtain the following characterization of boundedness.

%\begin{restatable}{theorem}{thmmaintheoremsphere} \label{maintheoremsphere}
%  Consider $\Sd{d}$, equipped with the operator $P_{\text{SH}}$. Then if $1/p - 1/2 > 1/(d-1)$, $s = (d-1)(1/p - 1/2)$, and $a$ is any regulated function, then $\| a \|_{M^p_{\text{Dil}}(\Sd{d})} \sim \| a \|_{R^{s,p}[0,\infty)}$.
%\end{restatable}
...
%
%\begin{itemize}
%    \item Consider what seems a relatively simple case, the sphere $\Sd{d}$ equipped with the Laplacian. Then $\Lambda = \{ k(k+d-1) : k \geq 0 \}$, and for $\lambda = k(k+d-1)$, the space $\mathcal{V}_\lambda$ consists of the spherical harmonics of degree $k$, restrictions of harmonic, homogeneous, degree $k$ polynomials on $\RR^{d+1}$ to $\Sd{d}$. There is a basis for such a space defined in terms of the associated Legendre polynomials, but this is still relatively non-explicit.
...
For any other value of $p$, $M^p_{\text{Rad}}(\RR^d)$ is a proper subset of $R^{s,p}[0,\infty)$:
%
\begin{itemize}
...
This completes a proof of the single scale estimates of the paper. The paper then uses an atomic decomposition method to combine frequency scales and thus complete the proof of Theorem \ref{HeoNazarovSeegerTheorem}. We discuss these methods in the following section.
...
\begin{lemma}[The Fundamental Inequality for Finsler Metrics]
...
\begin{lemma} \label{lema:wdoijwadoiawjdlabel}
...
\begin{proof}
  ...
  %
  % Maximize xi(v) such that F(v)^2/2 = F^*(xi)^2/2
  %
  % When is xi a multiple of F(v) Sum g_{ij}(x,v) v^j
  % when xi = a F(v) sum g_{ij}(x,v) v^j for some a
  % Since F^*( Sum g_{ij}(x,v) v^j ) = F(v) = F^*(xi)
  % a = 1/F^*(xi)
  ...
  % xi(f(xi)) = F^*(xi)^2
  %
  % Sum xi_a f(xi)^a = F^*(xi)^2
  % f(xi)^a + sum xi_i (partial_a f^i)
  % partial_b f^a + (partial_a f^b) + sum xi_i ( partial_a partial_b f^i ) = 2 g^{ab}
  % 
...
%  $v = (v^\sharp)_\flat$. Thus both maps are diffeomorphisms.

%   It follows that the map $\xi \mapsto \xi_\sharp$ is injective. Moreover, given any vector
...
%
% ((Nabla_v f) o c))'' = (Nabla_x f) o c
%
% If xi = Nabla_v f
%
% then Euler Lagrang is equivalent to
%
% x'(t) = Nabla_xi p
% xi'(t) = - Nabla_x p
%
% If v(t,x,xi) solves xi = Nabla_v f(t,x,v)
% and we define p(t,x,xi) = xi(v) - f(t,x,v)
% Then p solves the equation
%
% x' =


%\begin{corollary}
%  The norm $F_*$ is strictly convex, and if $\xi = v^\flat$, and we define
  %
%  \[ g^{ij}(x,\xi) = \frac{1}{2} \frac{\partial^2 F_*^2}{\partial \xi_i \partial \xi_j}(x,\xi). \]
  %
%  then
...
\begin{proof}
  ...
  \end{equation}
  %
  form a positive-definite matrix. But inverting the procedure of the previous lemma shows that the dual norm $F(x,v) = \sup\nolimits_{\xi \in S_x^*} \xi(v)$ is also strictly convex, and thus gives a Finsler metric on $X$ with $F_* = p$. %(Chapter 14 of \cite{BaoChern}). 
\end{proof}
...
The \emph{equivalence of phase theorem} for Fourier integral operators tells us that the particular phase used to define Fourier integral operators is largely irrelevant to the analysis of the operator, as long as the canonical relation is shared by the phase.
% To state the theorem, we must define what it means to view a Fourier integral operator `in coordinates'. Let $A$ be a Fourier integral operator from a manifold $X$ to a manifold $Y$, and consider open sets
%
%\[ U_X \Subset V_X \subset X \quad\text{and}\quad U_Y \Subset V_Y \subset Y, \]
%
%together with diffeomorphisms $F^X: V_X \to W_X$ and $F^Y: V_Y \to W_Y$, where $W_X$ and $W_Y$ are boudned subsets of $\RR^n$ and $\RR^m$. Consider smooth $\phi_X$ and $\phi_Y$, with $\text{supp}(\phi_X) \subset V_X$ and $\text{supp}(\phi_Y) \subset V_Y$, but with $\phi_X$ equal to one on $U_X$ and with $\phi_Y$ equal to one on $U_Y$. Define a Schwartz operator $A^{W_X,W_Y}$ from $W_Y$ to $W_X$ such that for $f \in C_c^\infty(W_Y)$,
%
%\[ A^{W_X,W_Y} f = A \{ f \circ F_Y \} \circ F_X^{-1}. \]
...
%
% Index(omega) = [omega:alpha] + Index[ Q( omega(b), alpha; omega(a) ) ]
%
% where Q is the symmetric bilinear form on omega(b) describing omega(a)
% Which is equal to 1/2( dim(omega(b)) - dim Ker(Q) - sgn(Q) )
%
% sigma is a symplectic form
% 
% 
% s(alpha,alpha',beta,beta') = sgn(Q())
%
% Index of curve is ind(omega) = [ omega: alpha ] + ind( Q( omega(b), alpha; omega(a) ) )
% 
